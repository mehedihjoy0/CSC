...
if grep -q "vendor.samsung.hardware.biometrics.face" "$MANIFEST" && grep -q "<version>2.0</version>" "$MANIFEST"; then
    ...
    if GET_FEATURE SOURCE_HAVE_LEGACY_FACE_HAL; then
        ...
        if GET_FEATURE DEVICE_HAVE_LEGACY_FACE_HAL; then
            ...
            ADD_FROM_FW "extra" "vendor" "bin/hw/vendor.samsung.hardware.biometrics.face@3.0-service"
            ADD_FROM_FW "extra" "vendor" "lib/vendor.samsung.hardware.biometrics.face@3.0.so"
            ADD_FROM_FW "extra" "vendor" "lib64/vendor.samsung.hardware.biometrics.face@3.0.so"
            ADD_FROM_FW "extra" "vendor" "etc/init/vendor.samsung.hardware.biometrics.face@3.0-service.rc"
